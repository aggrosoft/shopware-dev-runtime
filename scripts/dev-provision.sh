...
# Keep a system default and also seed the persistent user config because the
# IDE extension reads ~/.codex/config.toml directly. Existing user settings are
# preserved; only the missing Playwright server block is appended.
...
if ! grep -q '^\[mcp_servers\.playwright\]# Seed editable workspace instructions once. The backing file lives inside
